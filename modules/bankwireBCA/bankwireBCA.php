...
class BankWireBCA extends PaymentModule
{
	private $_html = '';
	...
	public function __construct()
	{
		...
		$config = Configuration::getMultiple(array('BCA_DETAILS', 'BCA_OWNER', 'BCA_ADDRESS'));
		if (isset($config['BCA_OWNER']))
			$this->owner = $config['BCA_OWNER'];
		if (isset($config['BCA_DETAILS']))
			$this->details = $config['BCA_DETAILS'];
		if (isset($config['BCA_ADDRESS']))
			$this->address = $config['BCA_ADDRESS'];

		parent::__construct();

		$this->displayName = $this->l('Bank Wire BCA');
		$this->description = $this->l('Accept payments by BCA (bank wire)');
	...
	{
		if (!Configuration::deleteByName('BCA_DETAILS')
				OR !Configuration::deleteByName('BCA_OWNER')
				OR !Configuration::deleteByName('BCA_ADDRESS')
				OR !parent::uninstall())
			return false;
	}
	...
	private function _postProcess()
	{
		if (isset($_POST['btnSubmit']))
		{
			Configuration::updateValue('BCA_DETAILS', $_POST['details']);
			Configuration::updateValue('BCA_OWNER', $_POST['owner']);
			Configuration::updateValue('BCA_ADDRESS', $_POST['address']);
		}
	...
	{
		$this->_html .= '<img src="../modules/bankwireBCA/bankwire.jpg" style="float:left; margin-right:15px;"><b>'.$this->l('This module allows you to accept payments by bank wire.').'</b><br /><br />
